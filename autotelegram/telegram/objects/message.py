...
if TYPE_CHECKING:
    ...
    from autotelegram.telegram.objects.audio import Audio
    from autotelegram.telegram.objects.animation import Animation
    from autotelegram.telegram.objects.chat import Chat
    from autotelegram.telegram.objects.contact import Contact
    from autotelegram.telegram.objects.dice import Dice
    from autotelegram.telegram.objects.document import Document
    from autotelegram.telegram.games.objects import Game
    from autotelegram.telegram.inline.objects import InlineKeyboardMarkup
    from autotelegram.telegram.objects.location import Location
    from autotelegram.telegram.objects.photosize import PhotoSize
    from autotelegram.telegram.objects.poll import Poll
    from autotelegram.telegram.objects.proximityalerttriggered import ProximityAlertTriggered
    from autotelegram.telegram.passport.objects import PassportData
    from autotelegram.telegram.payments.objects import SuccessfulPayment, Invoice
    from autotelegram.telegram.stickers.objects import Sticker
    from autotelegram.telegram.objects.user import User
    from autotelegram.telegram.objects.venue import Venue
    from autotelegram.telegram.objects.video import Video, VideoNote, VideoChatStarted, VideoChatEnded, VideoChatParticipantsInvited, VideoChatScheduled
    from autotelegram.telegram.objects.voice import Voice
...
class Message(BaseObject):
    ...
    async def reply_with_text (self,text,**kwargs):
        ...
        Args:
            text (str): The text message to reply with
            **kwargs: These are the same kwargs passed to the send_message method.
        """

        result = await self._current_context.send_message(chat_id = self.chat.id,
    ...
    async def respond_with_text (self,text,**kwargs):
        ...
        Args:
            text (str): The text message to respond with.
            **kwargs: These are the same kwargs passed to the send_message method
        """
    ...
    async def delete_message(self):
        ...
        Delete this message
        """
        await self._current_context.delete_message(chat_id = self.chat.id,
    ...
    async def replace_with_text (self,new_text,**kwargs):
        ...
        Args:
            new_text (str): the new text to replace old text with
            **kwargs: these are keyword arguments passed to the underlying `send_message` method
        """
